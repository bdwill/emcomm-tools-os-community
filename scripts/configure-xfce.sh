...
if command -v xfconf-query >/dev/null 2>&1; then
  # Wrap in dbus-run-session to handle headless environments (e.g. ISO builds)
  # Some versions of xfconf-query still need a DISPLAY even with dbus-run-session
  export DISPLAY=${DISPLAY:-:0}
  
  # Use a single dbus-run-session for efficiency
